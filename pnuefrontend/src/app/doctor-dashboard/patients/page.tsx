...
export default function DoctorPatientsPage() {
  ...
  const [patientTab, setPatientTab] = useState<"history" | "scans" | "prescriptions">("history");
  ...
  const fetchPatientRecords = async (patientId: number, authToken: string) => {
    ...
    try {
      ...
      setPatientScans(scansRes.data.filter((s: any) => s.patient_id === patientId));

      // Prescriptions
      const presRes = await axios.get(`http://localhost:8000/api/prescriptions/?patient_id=${patientId}`, { headers });
      setPatientPrescriptions(presRes.data);

      // History
      const histRes = await axios.get(`http://localhost:8000/api/medical-histories/?patient_id=${patientId}`, { headers });
    ...
    <div className="flex h-screen overflow-hidden bg-brand-surface font-sans">
      ...
      <div className="flex-1 flex flex-col pl-64 h-screen overflow-hidden">
        ...
        <main className="flex-1 overflow-hidden p-8 flex flex-col space-y-6">
          <div>
            <h1 className="text-3xl font-extrabold text-brand-navy mb-1">Clinic Patient Records</h1>
            <p className="text-brand-muted font-medium">
              Access active medical profiles, diagnostic radiograph catalogues, and write prescription charts.
            </p>
          </div>
          ...
          {loading ? (
            ...
            <div className="bg-brand-white border border-brand-border rounded-2xl p-12 text-center max-w-xl mx-auto flex flex-col items-center flex-1 justify-center">
              ...
              <h4 className="font-bold text-brand-navy text-sm">No registered patients in database</h4>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="overflow-hidden">
                          ...
                          <span className="text-[10px] text-brand-muted block truncate mt-0.5">{p.email}</span>
                        </div>
                      </button>
                    );
                  })}
                ...
                <div className="flex-1 bg-brand-white border border-brand-border rounded-2xl flex flex-col overflow-hidden shadow-soft">
                  ...
                  <div className="p-6 border-b border-brand-border bg-brand-white flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        ...
                        <span className="text-xs text-brand-muted font-bold uppercase tracking-wider">{selectedPatient.email}</span>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-1 text-xs text-brand-navy border-l border-brand-border/40 pl-6">
                      <p><strong className="text-brand-muted uppercase text-[9px] block">Age</strong> {selectedPatient.age ? `${selectedPatient.age} Yrs` : "—"}</p>
                      <p><strong className="text-brand-muted uppercase text-[9px] block">Gender</strong> <span className="capitalize">{selectedPatient.gender || "—"}</span></p>
                      <p><strong className="text-brand-muted uppercase text-[9px] block">Blood Group</strong> {selectedPatient.blood_group || "—"}</p>
                      <p><strong className="text-brand-muted uppercase text-[9px] block">Contact</strong> {selectedPatient.contact_number || "—"}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex border-b border-brand-border bg-brand-surface/30 px-6">
                    {[
                      ...
                      { id: "scans", label: "Scan Diagnostics", icon: Activity },
                      { id: "prescriptions", label: "Prescription log", icon: FileText }
                    ].map((tab) => {
                      const Icon = tab.icon;
                      return (
                        <button
                  ...
                  <div className="flex-1 overflow-y-auto p-6">
                    {recordsLoading ? (
                      ...
                      <>
                        ...
                        {patientTab === "history" && (
                          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            <div className="lg:col-span-2 space-y-4">
                              <h3 className="text-sm font-bold text-brand-navy border-b border-brand-border/60 pb-2 uppercase tracking-wider">Clinical Records History</h3>
                              {patientHistory.length === 0 ? (
                                <div className="text-center py-8 bg-brand-surface/30 rounded-xl border border-dashed border-brand-border">
                                  <p className="text-brand-muted text-xs font-semibold">No medical history logged.</p>
                                </div>
                              ) : (
                                <div className="space-y-4">
                                  {patientHistory.map((hist) => (
                                    <div key={hist.id} className="p-4 border border-brand-border rounded-xl bg-brand-surface/20 flex flex-col justify-between">
                                      <div className="flex justify-between items-start mb-2">
                                        <h4 className="font-bold text-brand-navy text-sm">{hist.condition}</h4>
                                        <span className="text-[10px] text-brand-muted font-bold flex items-center space-x-1">
                                          <Calendar className="w-3.5 h-3.5" />
                                          <span>Diagnosed: {hist.diagnosis_date}</span>
                                        </span>
                                      </div>
                                      {hist.treatment && (
                                        <p className="text-xs text-brand-navy mb-1"><strong className="text-brand-muted uppercase text-[9px] block">Treatment Protocol:</strong> {hist.treatment}</p>
                                      )}
                                      {hist.notes && (
                            ...
                            <div className="lg:col-span-1">
                              <div className="bg-brand-surface/30 border border-brand-border rounded-2xl p-5">
                                ...
                                <form onSubmit={handleAddHistory} className="space-y-4">
                                  <div>
                                    <label className="block text-[10px] font-bold text-brand-muted uppercase tracking-wider mb-1">Condition</label>
                                    <input
                                      type="text"
                                      required
                                  ...
                                  <div>
                                    <label className="block text-[10px] font-bold text-brand-muted uppercase tracking-wider mb-1">Diagnosis Date</label>
                                    <input
                                      type="date"
                                      required
                                  ...
                                  <div>
                                    <label className="block text-[10px] font-bold text-brand-muted uppercase tracking-wider mb-1">Treatment Plan</label>
                                    <input
                                      type="text"
                                  ...
                                  <div>
                                    <label className="block text-[10px] font-bold text-brand-muted uppercase tracking-wider mb-1">Clinical Notes</label>
                                    <textarea
                                      rows={2}
                        ...
                        {patientTab === "scans" && (
                          <div className="space-y-4">
                            <h3 className="text-sm font-bold text-brand-navy border-b border-brand-border/60 pb-2 uppercase tracking-wider">Radiograph Diagnosis Records</h3>
                            {patientScans.length === 0 ? (
                              <div className="text-center py-8 bg-brand-surface/30 rounded-xl border border-dashed border-brand-border">
                                <p className="text-brand-muted text-xs font-semibold">No scans conducted for this patient.</p>
                              </div>
                            ) : (
                              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {patientScans.map((scan) => (
                                  <div key={scan.id} className="border border-brand-border rounded-xl overflow-hidden hover:shadow-soft bg-brand-white flex flex-col">
                                    <div className="h-40 bg-slate-900 flex items-center justify-center overflow-hidden relative">
                                      {scan.image_url ? (
                                        <img src={scan.image_url} alt="Radiograph" className="h-full w-full object-cover" />
                                      ) : (
                                        <span className="text-brand-muted text-[10px]">No image preview</span>
                                      )}
                                      <span className={`absolute top-2.5 right-2.5 inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold border ${
                                        scan.result === "Normal" ? "bg-emerald-50 text-emerald-600 border-emerald-200" : "bg-rose-50 text-rose-600 border-rose-200"
                                      }`}>
                                        {scan.result}
                                      </span>
                                    </div>
                                    <div className="p-4 space-y-2">
                                      <div className="flex justify-between items-center text-[10px] text-brand-muted font-bold">
                                        ...
                                        <span>AI Conf: {(scan.confidence * 100).toFixed(1)}%</span>
                                      </div>
                        ...
                        {patientTab === "prescriptions" && (
                          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            <div className="lg:col-span-2 space-y-4">
                              <h3 className="text-sm font-bold text-brand-navy border-b border-brand-border/60 pb-2 uppercase tracking-wider">Issued Prescription Logs</h3>
                              {patientPrescriptions.length === 0 ? (
                                <div className="text-center py-8 bg-brand-surface/30 rounded-xl border border-dashed border-brand-border">
                                  <p className="text-brand-muted text-xs font-semibold">No prescriptions logged.</p>
                                </div>
                              ) : (
                                <div className="space-y-4">
                                  {patientPrescriptions.map((pres) => (
                                    <div key={pres.id} className="p-4 border border-brand-border rounded-xl bg-brand-surface/20">
                                      <div className="flex justify-between items-start mb-2">
                                        <h4 className="font-bold text-brand-navy text-sm">{pres.medication}</h4>
                                        <span className="text-[10px] text-brand-muted font-bold">Issued: {new Date(pres.date_issued).toLocaleDateString()}</span>
                                      </div>
                                      <p className="text-xs text-brand-navy"><strong className="text-brand-muted uppercase text-[9px]">Dosage:</strong> {pres.dosage}</p>
                                      <p className="text-xs text-brand-navy mt-1"><strong className="text-brand-muted uppercase text-[9px] block">Instructions:</strong> {pres.instructions}</p>
                                      {pres.doctor_notes && (
                                        <div className="text-[11px] text-brand-muted mt-2 pt-2 border-t border-brand-border/40">
                                          <strong className="text-brand-navy">Attending Notes:</strong> {pres.doctor_notes}
                                        </div>
                                      )}
                                    </div>
                                  ))}
                            ...
                            <div className="lg:col-span-1">
                              <div className="bg-brand-surface/30 border border-brand-border rounded-2xl p-5">
                                ...
                                <form onSubmit={handleAddPrescription} className="space-y-4">
                                  <div>
                                    <label className="block text-[10px] font-bold text-brand-muted uppercase tracking-wider mb-1">Medication Name</label>
                                    <input
                                      type="text"
                                      required
                                  ...
                                  <div>
                                    <label className="block text-[10px] font-bold text-brand-muted uppercase tracking-wider mb-1">Dosage</label>
                                    <input
                                      type="text"
                                      required
                                  ...
                                  <div>
                                    <label className="block text-[10px] font-bold text-brand-muted uppercase tracking-wider mb-1">Attending Instructions</label>
                                    <textarea
                                      rows={2}
                                      required
                                  ...
                                  <div>
                                    <label className="block text-[10px] font-bold text-brand-muted uppercase tracking-wider mb-1">Doctor Notes (Optional)</label>
                                    <textarea
                                      rows={2}
