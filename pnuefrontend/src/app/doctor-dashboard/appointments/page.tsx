...
export default function DoctorAppointmentsPage() {
  ...
  const handleMarkComplete = async (appId: number) => {
    ...
    try {
      await axios.patch(
        ...
        { action: "complete", doctor_notes: "Appointment successfully conducted." },
        { headers: { Authorization: `Token ${token}` } }
      );
      fetchAppointments(token);
    } catch (err) {
  ...
  // Counters
  const countPending = appointments.filter((app) => app.status === "Pending").length;
  const countAccepted = appointments.filter((app) => app.status === "Accepted").length;
  const countCompleted = appointments.filter((app) => app.status === "Completed").length;
  const countRejected = appointments.filter((app) => app.status === "Rejected").length;
    ...
    <div className="flex h-screen overflow-hidden bg-brand-surface font-sans">
      ...
      <div className="flex-1 flex flex-col pl-64 h-screen overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-8 space-y-8">
          <div>
            <h1 className="text-3xl font-extrabold text-brand-navy mb-1">Clinic Appointments Dashboard</h1>
          ...
          <div className="flex space-x-2 border-b border-brand-border pb-px">
            {[
              ...
              { id: "Accepted", label: "Scheduled & Accepted", count: countAccepted },
              { id: "Completed", label: "Completed Checkups", count: countCompleted },
              { id: "Rejected", label: "Rejected Requests", count: countRejected }
            ].map((tab) => (
              <button
                key={tab.id}
          ...
          {loading ? (
            ...
            <div className="bg-brand-white border border-brand-border rounded-2xl p-12 text-center max-w-xl mx-auto flex flex-col items-center">
              ...
              <h4 className="font-bold text-brand-navy text-sm">No appointment requests in this tab</h4>
              <p className="text-xs text-brand-muted mt-1">
                Patients can schedule pulmonary checkup slots via their personal patient clinical dashboards.
              </p>
            </div>
          ) : (
                ...
                >
                  <div className="space-y-4">
                    <div className="flex justify-between items-start">
                      <div className="flex items-center space-x-2">
                        ...
                        <div>
                          ...
                          <span className="text-[10px] text-brand-muted font-bold block">{app.patient_email}</span>
                        </div>
                      </div>
                      <span className={`px-2 py-0.5 rounded text-[10px] font-extrabold uppercase border ${
                        ...
                        app.status === "Rejected" ? "bg-rose-50 text-rose-700 border-rose-200" : "bg-brand-surface text-brand-muted"
                      }`}>
                        {app.status}
                      </span>
                    </div>
                    ...
                    <div className="bg-brand-surface/40 p-4 rounded-xl border border-brand-border/40 text-xs font-semibold text-brand-navy space-y-2">
                      <p><span className="text-brand-muted uppercase text-[9px] block">Requested Slots:</span> {app.requested_date} @ {app.requested_time}</p>
                      <p><span className="text-brand-muted uppercase text-[9px] block font-bold">Reason for checkup:</span> {app.reason}</p>
                      {app.notes && (
                    ...
                    {app.status === "Accepted" && (
                      <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-3 text-xs font-bold text-emerald-800 space-y-1">
                        ...
                        <p className="text-brand-navy font-black pl-5">Date: {app.appointment_date} at {app.appointment_time}</p>
                      </div>
                    )}

                    {app.doctor_notes && (
                      <div className="p-3 bg-brand-indigo/5 border border-brand-indigo/10 rounded-lg text-xs">
                        <strong className="text-brand-navy uppercase text-[9px] block mb-0.5">attending clinician notes:</strong>
                        <p className="italic text-brand-navy">"{app.doctor_notes}"</p>
                      </div>
                    )}
                  </div>
              ...
              <div className="bg-brand-white rounded-2xl max-w-md w-full border border-brand-border shadow-2xl overflow-hidden animate-in zoom-in duration-200">
                ...
                <form onSubmit={handleAction} className="p-6 space-y-4">
                  <div className="bg-brand-surface/40 p-4 rounded-xl border border-brand-border/40 text-xs font-semibold text-brand-navy space-y-1">
                    <p><span className="text-brand-muted uppercase text-[9px] block">Patient Profile</span> {selectedApp.patient_name || selectedApp.patient_email} ({selectedApp.patient_email})</p>
                    <p><span className="text-brand-muted uppercase text-[9px] block">Requested Time</span> {selectedApp.requested_date} at {selectedApp.requested_time}</p>
                    <p className="italic text-brand-muted font-medium mt-1">Reason: "{selectedApp.reason}"</p>
                  </div>

                  {modalAction === "accept" && (
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs font-bold text-brand-muted uppercase tracking-wider mb-1.5">Confirmed Date</label>
                        <input
                          type="date"
                          required
                          value={modalDate}
                      ...
                      <div>
                        <label className="block text-xs font-bold text-brand-muted uppercase tracking-wider mb-1.5">Confirmed Time</label>
                        <input
                          type="time"
                          required
                          value={modalTime}
                  ...
                  <div>
                    <label className="block text-xs font-bold text-brand-muted uppercase tracking-wider mb-1.5">
                      {modalAction === "accept" ? "Attending Clinician Guidelines" : "Optional Rejection Reason"}
                    </label>
                    <textarea
                      rows={3}
                      required={modalAction === "reject"}
                      placeholder={modalAction === "accept" ? "e.g. Please bring past medical scan copies." : "State the reason for declining checkup schedule..."}
                      value={modalNotes}
                  ...
                  <div className="flex justify-end space-x-2 pt-4 border-t border-brand-border/40 mt-6">
                    ...
                    <button
                      ...
                      className={`px-5 py-2.5 rounded-lg text-xs font-bold text-white shadow-md active:scale-98 transition-colors flex items-center space-x-1.5 ${
                        modalAction === "accept" ? "bg-brand-indigo hover:bg-[#2a2853]" : "bg-rose-600 hover:bg-rose-700"
                      }`}
                    >
                      {modalLoading ? (
                        <Loader2 className="w-3.5 h-3.5 animate-spin" />
                      ) : (
                        modalAction === "accept" ? "Confirm Booking" : "Decline Booking"
                      )}
                    </button>
                  </div>
                </form>
